...
interface TradeinFieldEditDialogProps {
  isOpen: boolean;
  onClose: () => void;
  configId: string;
  field: any;
...
const TradeinFieldEditDialog: React.FC<TradeinFieldEditDialogProps> = ({
  isOpen,
  onClose,
  configId,
  field
  ...
  const { data: dropdowns } = useQuery({
    queryKey: ['dropdowns-for-tradein-field-edit'],
    queryFn: async () => {
  ...
  const updateTradeinFieldMutation = useMutation({
    mutationFn: ({ configId, fieldId, data }: { configId: string; fieldId: string; data: any }) =>
      configServices.updateTradeinField(configId, fieldId, data),
    onSuccess: () => {
      toast.success('Trade-in field updated successfully');
      onClose();
      queryClient.invalidateQueries({ queryKey: ['tradein-config-details'] });
    },
    onError: (error: any) => {
      toast.error(error.response?.data?.message || 'Failed to update trade-in field');
    },
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    updateTradeinFieldMutation.mutate({
      configId,
  ...
  // Fixed: Corrected the typo in field types
  const fieldTypes = [
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit Trade-in Field</DialogTitle>
          <DialogDescription>
            Modify the trade-in field properties and configuration
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end space-x-2">
            ...
            <Button type="submit" disabled={updateTradeinFieldMutation.isPending}>
              {updateTradeinFieldMutation.isPending ? 'Updating...' : 'Update Field'}
            </Button>
          </div>
        </form>
      </DialogContent>
...
export default TradeinFieldEditDialog;