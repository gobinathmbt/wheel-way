...
import VehicleSafetySection from "@/components/vehicles/VehicleSections/PricingSections/VehicleSafetySection";
...
const VehicleTradeSideModal: React.FC<VehicleTradeSideModalProps> = ({
  ...
  useEffect(() => {
    if (
      ...
      const stages = vehicle.trade_in_result
  ...
  const handlePushToWorkshop = async () => {
    if (vehicle.vehicle_type === "tradein") {
      ...
      setPendingAction({ type: "inspection" });
      setConfirmationOpen(true);
    }
  };

  // Update the handleConfirmAction function to immediately update local state
  const handleConfirmAction = async () => {
    try {
      ...
      if (pendingAction.type === "inspection") {
        ...
        onUpdate();
      } else if (pendingAction.type === "tradein" && pendingAction.stages) {
        await vehicleServices.updateVehicleWorkshopStatus(
          vehicle._id,
          vehicleType,
          {
    ...
    <>
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            <SheetTitle>Select Trade-in Stage</SheetTitle>
            <SheetDescription>
              Choose which trade-in stage report you want to view
            </SheetDescription>
          </DialogHeader>
