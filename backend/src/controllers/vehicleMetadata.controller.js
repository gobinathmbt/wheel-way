...
const modifyController = {
  ...
  update: async (req, res) => {
    try {
      ...
      // Populate if it's model or metadata
      if (type === 'model') {
        await result.populate('make', 'displayName displayValue');
      ...
      switch (type) {
        case 'make':
          ...
          dependencyChecks = [
            ...
            { model: VehicleMetadata, field: 'make', message: 'vehicle metadata' }
          ];
        ...
        case 'model':
          Model = Model;
          dependencyChecks = [
            { model: VehicleMetadata, field: 'model', message: 'vehicle metadata' }
          ];
        ...
        case 'body':
          Model = Body;
          dependencyChecks = [
            { model: VehicleMetadata, field: 'body', message: 'vehicle metadata' }
          ];
        ...
        case 'year':
          Model = VariantYear;
          dependencyChecks = [
            { model: VehicleMetadata, field: 'variantYear', message: 'vehicle metadata' }
          ];
...
const retrieveController = {
  ...
  list: async (req, res) => {
    try {
      ...
      // Execute query
      let query = Model.find(filter).sort(sortCriteria).skip(skip).limit(parseInt(limit));
  ...
  dropdown: async (req, res) => {
    try {
      ...
      switch (type) {
        ...
        case 'models':
          const filter = makeId ? { make: makeId, isActive: true } : { isActive: true };
