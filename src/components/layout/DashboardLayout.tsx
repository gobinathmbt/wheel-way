 ...
 useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (popoverContent && !popoverContent.contains(event.target as Node) && 
          popoverTrigger && !popoverTrigger.contains(event.target as Node)) {
        setClickedMenu(null);
      }
    };
...
const renderCollapsedMenuItem = (item: NavigationItem, index: number) => {
            ...
            <TooltipTrigger asChild>
              <div>
                <Popover open={isOpen} onOpenChange={(open) => {
                  ...
                  <PopoverContent side="right" className="w-48 p-2" sideOffset={8} align="start">
  ...
  const renderNavigationItem = (item: NavigationItem, index: number) => {
    ...
    if (item.children && item.children.length > 0) {
      ...
      return (
        ...
        >
          <CollapsibleTrigger className={`w-full flex items-center rounded-lg transition-all duration-200 p-3 space-x-3 justify-between ${
            isActive 
              ? 'bg-primary text-primary-foreground' 
              : 'text-muted-foreground hover:bg-accent hover:text-accent-foreground'
          }`}>
  ...
  const Sidebar = ({ className = '' }) => (
    <div className={`flex flex-col h-full bg-card border-r transition-all duration-300 ease-in-out ${className} ${
      ...
      <nav className="flex-1 px-2 space-y-1 overflow-y-auto py-4">
        {navigationItems.length > 0 ? (
          ...
          <div className={`text-center p-4 text-muted-foreground transition-all duration-300 ${
