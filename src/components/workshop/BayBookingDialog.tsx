      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        {isLoading ? (
          ...
          <div className="space-y-6 py-4">
            ...
            {/* All Bays Grid - Only show if no bay is selected */}
            {!selectedBayId && (
              <div>
                <h3 className="text-sm font-semibold mb-3 text-muted-foreground">
                  Or browse all available bays:
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {baysData.map((bay: any) => (
                    <Card
                      key={bay._id}
                      className="hover:shadow-lg transition-shadow cursor-pointer border hover:border-primary/50"
                      onClick={() => setSelectedBayId(bay._id)}
                    >
                        ...
                        <div className="space-y-2">
                          ...
                          {bay.bay_timings && (
                            <div className="flex items-center gap-2 text-xs">
                              ...
                              <span>
                                {getWorkingDaysCount(bay.bay_timings)}/7 Days •{" "}
                                {getBayTimingsSummary(bay.bay_timings)}
                              </span>
                            </div>
                          )}
