  ...
  const generatePdf = async () => {
    ...
    try {
      ...
      const addFooter = (pageNum: number, totalPages: number) => {
        ...
        pdf.text(`Report generated by Inspector ID: ${inspectorId}`, margin, footerY);
      ...
      // Add image with proxy
      const addImageWithProxy = async (url: string, x: number, y: number, width: number, height: number) => {
        try {
          const proxyUrl = `https://app-backend.securegateway.io/api/proxy/image?url=${encodeURIComponent(url)}`;
          pdf.addImage(proxyUrl, "JPEG", x, y, width, height);
          return true;
        } catch (error) {
          console.error('Error adding image:', error);
          pdf.setFontSize(8);
          pdf.setTextColor(200, 0, 0);
          pdf.text('Image failed to load', x, y + height/2);
          return false;
        }
      };
      ...
      pdf.text(`Inspector ID: ${inspectorId}`, margin + 5, currentY + 5);
      currentY += 15;
      ...
      const processFields = async (fields: any[], formData: any, formNotes: any, formImages: any, formVideos: any) => {
        for (const field of fields) {
          ...
          // Inspector ID for field
          pdf.setFontSize(7);
          pdf.setTextColor(148, 163, 184);
          pdf.text(`Inspected by: ${inspectorId}`, margin + 5, currentY);
          currentY += 4;
          ...
          if (notes) {
            ...
            pdf.rect(margin + 5, currentY - 2, contentWidth - 10, notesHeight + 4, 'F');
            pdf.setFontSize(8);
            pdf.setFont('helvetica', 'italic');
            pdf.setTextColor(161, 98, 7);
            pdf.text('Notes:', margin + 7, currentY + 2);
            pdf.setTextColor(71, 85, 105);
            currentY = addWrappedText(notes, margin + 7, currentY + 4, contentWidth - 24, 8, 'normal', 0.35);
            currentY += 2;
          }
          ...
          if (images.length > 0) {
            ...
            // Add first image (larger)
            const imgWidth = 60;
            const imgHeight = 40;
            checkPageBreak(imgHeight + 5);
            
            await addImageWithProxy(images[0], margin + 5, currentY, imgWidth, imgHeight);
            
            // Add indication if there are more images
            if (images.length > 1) {
              pdf.setFontSize(7);
              pdf.setTextColor(100, 116, 139);
              pdf.text(`+${images.length - 1} more images`, margin + 5 + imgWidth + 5, currentY + imgHeight/2);
            }
          ...
          if (videos.length > 0) {
            ...
            videos.forEach((video: string, index: number) => {
              checkPageBreak(5);
              pdf.textWithLink(`Video ${index + 1}: ${video}`, margin + 10, currentY, { url: video });
              currentY += 4;
            });
          }
    ...
    switch (field.field_type) {
      ...
      case 'multiplier':
        if (typeof value === 'object' && value !== null) {
          return `$${Number(value.total || 0).toFixed(2)}`;
        }
                ...
                <div ref={reportRef} className="bg-white p-8 rounded-lg shadow-sm border max-w-4xl mx-auto">
                  ...
                  <div className="text-center mb-8 pb-6 border-b border-gray-200">
                    ...
                    <div className="mt-4 flex items-center justify-center text-sm text-gray-600">
                      ...
                      Report generated by Inspector ID: {inspectorId}
                    </div>
                  </div>
                                              ...
                                              <div className="flex flex-wrap gap-2">
                                                ...
                                                {images.length > 6 && (
                                                  <div className="w-20 h-20 border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center">
                                                    <span className="text-xs text-gray-500">+{images.length - 6} more</span>
                                                  </div>
                                                )}
                                              </div>
                                            </div>
                                              ...
                                              <ul className="list-none space-y-1">
                                                {videos.map((video: string, index: number) => (
                                                  <li key={index} className="text-sm text-blue-600 hover:text-blue-800">
                                                    <a href={video} target="_blank" rel="noopener noreferrer" className="flex items-center">
                  ...
                  <div className="mt-12 pt-6 border-t border-gray-200 text-center text-xs text-gray-500">
                    ...
                    <p className="mt-1">Inspector ID: {inspectorId}</p>
